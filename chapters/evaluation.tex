...
In previous cold start test it became apparent, that running OpenWhisk vanilla with Docker on the Raspberry Pi is challenging. Because OpenWhisk runs four containers under heavy load, it puts too much strain on our test edge device. Thus, we make use of the previously introduced concurrency limit that we can set for each action. We set the limit to 10, such that 10 actions can be executed concurrently in the same container. With this setup, we can run the load test for the Pi because OpenWhisk only creates two containers -- at least up to a certain threshold. We use the same parameters for creating the Wasm actions. With that we can get the result in figure \ref{fig:pi-load-mixed}. OpenWhisk vanilla manages to handle up to 10 concurrent requests, at which point it creates a third container, which forces the Pi to use its swap memory and become orders of magnitude slower. Hence, at this point we stopped testing. The workload we run is mixed and made up of an equal amount of I/O- and CPU-bound actions. It is run in pseudo-random order, such that each test execution with the same parameters will put the same load on the container runtime under test.

We can see a stronger difference in all container runtimes than in the cold start test. \inl{wasmer} is 3.7x faster than docker, \inl{wasmtime} by about 3x and \inl{wamr} by 2.3x. The latter is the most consistent across the tests, whereas \inl{wasmtime} and \inl{wasmer} have a high fluctuation depending on the number of concurrent requests.

\begin{figure}
    \begin{center}
        \input{figures/pi-load-mixed.pgf}
    \end{center}
    \caption{The throughput with an equal amount of I/O- and CPU-bound workload on a Raspberry Pi with both Docker and Wasm-based container runtimes.}
    \label{fig:pi-load-mixed}
\end{figure}

To get a more isolated picture of the container runtimes performance, we test the workloads separately on our Desktop machine. The I/O-bound test is supposed to test the container runtimes' ability to handle concurrent network requests. As we've explored in chapter \ref{chapter:background}, serverless functions are frequently used with external services, such as databases. This test emulates this behaviour by blocking for 300ms, implemented using a simple thread sleep. In the Wasm executors, this is implemented with the capabilities, which allows the Wasm runtimes to supply native host functions as imports to the module. The module can then use these functions as if they were part of the module. This way, we could easily implement an actual blocking HTTP request. However, the lack of interface types means the module can only call native functions with integer parameters and return values, such that the practical use is still inhibited.
...
In figure \ref{fig:pc-load-hash} is the result of the fully CPU-bound load test. This test shows a larger difference between the Wasm executors compared to previous tests. On average, the Wasm executors process only 38\% (\inl{wamr}), 59\% (\inl{wasmtime}) and 84\% (\inl{wasmer}) of the throughput that Docker achieves.
