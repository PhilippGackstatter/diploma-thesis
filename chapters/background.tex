...
To that end, Wasm uses fault isolation techniques to sandbox the executing module. Interaction with the host environment is only possible through imported functions \cite{W3C2020}.

Wasm calls its heap linear memory. This memory is separate from the code space, which prevents programs from overwriting instructions. Programs can only operate in their own execution environment, but cannot escape. This means Wasm runtimes can safely execute multiple untrusted modules with their own linear memories in the same process memory space and without requiring additional isolation \cite{Haas2017}.

Wasm also features control-flow integrity because it enforces structured control flow. By design it is impossible to jump to arbitrary locations and the jump targets as well as other properties can be validated in a single-pass. This validation must happen before a module is executed by a runtime. The import mechanism represents a safe foreign function interface through which modules can call host functions.

% That makes Wasm very suitable for serverless execution as only a single instance of a runtime is necessary to execute potentially many serverless functions.
