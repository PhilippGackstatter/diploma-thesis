...
% subsection on Serverless Architectures or the Function-as-a-Service model
% -> some historical stuff
% -> its positioning among the other cloud computing models (PaaS, SaaS)
% -> application scenarios (which you will expand more on in Section 2.2 (serverless workloads)
% -> and even some high-level figure of a FaaS system.
% This would make the transition to section 2.1.1 smoother--i.e., you provide some more background on FaaS before jumping to the cold start problem). 
...
Managed services or Backend-as-a-Service (BaaS) are frequently used together with serverless, due to serverless functions being stateless. An example is AWS AppSync\footnote{\url{Serverless is frequently used together with }}, a managed GraphQL API, where AWS Lambdas can resolve fields of a GraphQL type, for example, by querying multiple databases at once.
