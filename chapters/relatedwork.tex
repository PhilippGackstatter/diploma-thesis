...
We did not provide a resource limitation since we believe our threading model to be only an intermediate solution until WebAssembly can be executed asynchronously, but it could use the same \inl{cgroup} mechanism in principle. When executing asynchronously, however, \inl{cgroups} cannot be used to limit an individual function's CPU time, which is thus an open problem.
