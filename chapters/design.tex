...
% Need to decide whether OW handles containers life cycle or if executor takes that responsibility
...
% TODO: Try to figure out from the Azure data set how many functions execute within 10-100ms and how many <= 10ms to support that point.
...
% In section \ref{section:wasm-runtimes}, we provided an overview of different runtimes and their execution models. 

\subsection{The WasmRuntime abstraction}

% Main function, async-std, code-snippet, concurrency, state sharing
% Capabilities
% Describe rough idea of what the architecture is
% - Main function, what init and run do
% - The WasmRuntime abstraction
% - Caching optimization
% - The precompiling optimization
% No module is shared internally between two containers. An inefficiency of the design - i.e. two containers with the same module code will store it twice. However, we should be able to set OW's concurrency to > 1, and then OW will send the request for the same module to that same container, but internally we will create a new instance of the module per-request, so we have isolation/sandboxing. And all that is efficient without having to change OW's design.
...
The executor decodes the base64 string given by OpenWhisk and unzips ist, before calling \inl{initialize} with the result. The function initializes the container identified by the given id. In \inl{wasmtime} and \inl{wasmer}, that means deserializing the 
...
\inl{run} executes the module associated with the given container id and with the parameters as input. In line with OpenWhisk, the parameters are a JSON object, which allows for a high degree of flexibility for the function input. Across all runtimes, this function looks up the previously stored module with the container id, creates a WASI context with the associated capabilities and executes the module. If an error occurs during the Wasm runtime setup, the outer \inl{Result} contains an \inl{anyhow::Error}, a generic, application-level error type. Since the Wasm module itself can also either succeed or fail, another \inl{Result} is nested inside, where both success and failure are represented by JSON objects.

\subsubsection{Cleanup}
Finally, the \inl{destroy} function removes the container, which simply means freeing the memory taken up by the module.

% The run function returns Rust's \inl{Result} type. It is an enum, where its \inl{Ok()} variant represents success, and its \inl{Err()} variant represents an error.

To our \inl{/init} endpoint, OpenWhisk sends ...



\inl{tide} will inject this state into each request, so it must be thread-safe.
